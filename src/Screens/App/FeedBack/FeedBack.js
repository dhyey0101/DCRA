...
const FeedBack = ({ navigation }) => {
  ...
  const submitFeedback = async () => {
    ...
    if (firstName == '') {
      dropDownAlertRef.alertWithType('error', 'DCRA', 'Please enter firstname');
      return;
    }
    if (email == '') {
      dropDownAlertRef.alertWithType('error', 'DCRA', 'Please enter email');
      return;
    }
    ...
    if (value === 0) {
      ...
      if (sendFeedData.status == 200) {
        ...
        setFirstName(''),
        setComment(''),
        dropDownAlertRef.alertWithType('success', 'DCRA', 'Feedback submitted successfully');
