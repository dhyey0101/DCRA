...
const socialLoginAPI = (deviceID, deviceName, FCM_Token, type, social_id, email) => {
  console.log(FCM_Token,"<<<<<<<<<FCM")
  // let responseObj = { error: 1, msg: '', result: '' };
  const data = {
    device_id: deviceID,
  ...
  console.log('url====', Config.baseUrl + APIConstants.social_Login, '=\n=', data);
  return postAPI(
    ...
    .catch(function (error) {
      // handle error
      const socialLoginAPI = error?.response?.data ? error.response.data : error;
